...
class _WeaponImpl(object):
    ...
    def _createWeaponStep(
        ...
        stepName = f'{componentString} - {weaponData.name()}'
    ...
    def _createMountStep(
        ...
        stepName = f'{componentString} - Mounting ({mountSize.value})'
        linkedGroupSize = self.linkedGroupSize()
        if linkedGroupSize:
            stepName += f' - x{linkedGroupSize.value()}'
    ...
    def _createAutoloaderStep(
        ...
        stepName = f'{componentString} - Autoloader {autoloaderMagazineCount.value()}'
        linkedGroupSize = self.linkedGroupSize()
        if linkedGroupSize:
            stepName += f' - x{linkedGroupSize.value()}'
    ...
    def _createFireControlStep(
        ...
        stepName = f'{componentString} - Fire Control System ({fireControl.value})'
        step = robots.RobotStep(
            name=stepName,
            type=typeString) 
...
class _ManipulatorWeaponImpl(_WeaponImpl):
    ...
    def _allowedMountSizes(
        ...
        for mountSize in allowed:
            minSize = _MinManipulatorSizeData[mountSize]
            if not minSize:
...
class _ManipulatorMountedWeaponImpl(_ManipulatorWeaponImpl):
    def isCompatible(
        ...
        manipulators = _enumerateManipulators(
            sequence=sequence, 
            context=context)        
        minUsableSize = _MinManipulatorSizeData[traveller.WeaponSize.Small]
...
class HandHeldFireControl(robots.HandHeldFireControlInterface):
    ...
    def componentString(self) -> str:
        return 'Handheld Weapon Fire Control'
    ...
    def createSteps(
        ...
        # TODO: The step generated here has a horrible name as it says 'Fire Control System' twice
        self._impl.createSteps(
            typeString=self.typeString(),
            componentString=self.componentString(),
            sequence=sequence,
