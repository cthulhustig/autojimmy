...
# TODO: Need to add something covering fact render options are set at the point the posters are generated
...
_WelcomeMessage = """
    ...
    metadata to {name}. Custom sector data will be merged with the existing data from Traveller Map to
    give a consistent view of your Traveller universe. This allows features such as jump route planning
    and trade calculations to take your sectors into account. The custom sector data is also uploaded
    to Traveller Map in order to generate posters of your custom sector. These posters are used by
    {name} to rewrite the tiles generated by Traveller map, overlaying your custom sectors to allow
    them to be displayed (almost) seamlessly in the integrated Traveller Map controls.</p>
    <p>There are a few important things to note about this feature:
    <ul style="margin-left:15px; -qt-list-indent:0;">
    <li>There are no tools for actually creating the sector and metadata files, this is left up to the
    user. Supported formats are:
    <ul style="margin-left:15px; -qt-list-indent:0;">
    ...
    <li>The overlaying of custom sector posters onto tiles generated by Traveller Map is not perfect. You
    may see some graphical artifacts around the borders where custom sectors meet other sectors. You may
    also notice some pixelisation at high zoom levels.</li>
    </ul></p>
    </html>
