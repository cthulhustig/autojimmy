...
class TagLevelFiler(WorldFilter):
    ...
    def match(
        ...
        tagLevel = tagging.calculateWorldTagLevel(world=world)
...
class ZoneFiler(WorldFilter):
    ...
    def description(self) -> str:
        ...
        zone = astronomer.zoneTypeDescription(self._value)
        if not zone:
            zone = 'Unknown'
        return f'Zone {operationString} {zone}'
...
class TradeCodeFilter(WorldFilter):
    ...
    def match(
        ...
        for tradeCode in checkList:
            match = world.hasTradeCode(tradeCode=tradeCode)
            if self._operation == ListFilterOperation.ContainsAny:
                if match:
                    return True
            elif self._operation == ListFilterOperation.ContainsAll:
